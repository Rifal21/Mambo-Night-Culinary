<div class="p-6 max-w-md md:max-w-[75%] md:ml-72 bg-[#F8F8F8] rounded-lg shadow-xl mt-5">
    ...
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-lg font-bold text-gray-700 border-b-2 border-secondary">Menu</h2>
            ...
            <li
                ...
                <div class="flex space-x-6">
                    <button class="text-blue-500 hover:text-blue-600"
                        onclick="openModalUpdateMenu({{ json_encode($item) }})">
                        <i class="fas fa-pen"></i>
                    </button>
                    <button class="text-yellow-500 hover:text-yellow-600"
                        onclick="openModalViewMenu({{ json_encode($item) }})">
                        <i class="fas fa-eye"></i>
                    </button>
                    <form action="/admin/menu-list/{{ $item->id }}" method="POST" class="d-inline">
                        @method('DELETE')
                        @csrf
...
<div id="modalCreate" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center hidden">
    <div class="bg-white rounded-lg shadow-lg w-80 h-[400px] p-6 relative overflow-hidden">
        ...
        <div class="overflow-y-auto no-scrollbar h-[320px]">
            <form action="/admin/menu-list" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <!-- Image Preview -->
                <div id="imagePreviewCreateMenu"
                    class="w-full h-40 bg-gray-100 rounded-lg flex justify-center items-center overflow-hidden mb-4">
                    <span class="text-gray-500">No Image Selected</span>
                </div>

                <button type="submit"
                    class="w-full bg-green-500 hover:bg-green-600 text-white font-medium rounded-lg px-4 py-2">
                    Tambah
                </button>
            </form>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-lg w-80 p-6 relative">
        ...
        <div class="overflow-y-auto h-[320px]">
            <form action="" method="POST" enctype="multipart/form-data" id="updateFormMenu">
                ...
                <label class="block text-gray-700 mb-2">Nama Menu</label>
                <input type="text" name="name" id="updateNameMenu"
                    class="border border-gray-300 p-2 w-full mb-4" />
                <label class="block text-gray-700 mb-2">Harga</label>
                <input type="text" name="harga" id="updateHargaMenu"
                    class="border border-gray-300 p-2 w-full mb-4" />

                <label class="block text-gray-700 mb-2">Gambar</label>
                <input type="file" name="gambar" id="updateGambarMenu"
                    class="border border-gray-300 p-2 w-full mb-4" onchange="previewUpdateImage(event)" />

                <div id="updateImageMenuPreview"
                    class="w-full h-40 bg-gray-100 rounded-lg flex justify-center items-center overflow-hidden mb-4">
                    <span class="text-gray-500">No Image Selected</span>
                </div>
...
<script>
    ...
    function openModalViewMenu(item) {
        ...
        harga.innerHTML =
            `<i class="fas fa-dollar-sign mr-2"></i>: Rp. ${new Intl.NumberFormat('id-ID').format(item.harga)}`;
        // Show the modal
    ...
    function openModalUpdateMenu(item) {
        ...
        updateForm.action = `/admin/menu-list/${item.id}`;

  ...
  <div class="px-3 py-3 lg:px-5 lg:pl-3">
    <div class="flex items-center justify-between">
      <div class="flex items-center justify-start rtl:justify-end">
        <button data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar" type="button" class="inline-flex items-center p-2 text-sm text-white rounded-lg sm:hidden hover:bg-secondary">
        ...
        <a href="/" class="flex ms-2 md:me-24" target="_blank">
          ...
          <span class="self-center text-xl font-semibold sm:text-2xl whitespace-nowrap dark:text-white">Mambo Kuliner</span>
        </a>
      </div>
          ...
          <div class="flex items-center ms-3">
            <div>
              <button type="button" class="flex text-sm bg-gray-800 rounded-full focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" aria-expanded="false" data-dropdown-toggle="dropdown-user">
                ...
                <img class="w-8 h-8 rounded-full" src="{{ Auth::user()->foto ? Auth::user()->foto : '/image/profil.png' }}" alt="user photo">
              </button>
            </div>
...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-primary dark:border-primary" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-primary dark:bg-primary">
      <ul class="space-y-2 font-medium">
        @foreach ($navItems as $item)         
        <li>
           <a href="{{ $item['link'] }}" class="flex items-center p-2 text-secondary rounded-lg dark:text-secondary hover:bg-gray-100 dark:hover:bg-secondary/20 group">
